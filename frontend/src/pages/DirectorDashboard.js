...
const DirectorDashboard = () => {
  ...
  const handleApprove = async (id) => {
    ...
    try {
      ...
      toast.success('Academic grievance approved and marked resolved');
      loadData();
    } catch (err) {
  ...
  const downloadCSV = () => {
    ...
    a.href = url;
    a.download = `grievances-director-${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
  ...
  const statusBadge = (st) => React.createElement('span', { className: `badge badge-${st}` }, st?.replace(/_/g, ' '));
  ...
  const academic = allGrievances.filter(g => g.category === 'academic');
      ...
      <main className="main-content">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              <div>
                ...
                <p className="page-sub" style={{ fontSize: 15 }}>
                  {user?.name} &nbsp;·&nbsp; <strong>Director — Academics &amp; Planning</strong>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="stats-grid" style={{ marginBottom: 28 }}>
          {[
            ...
            { label: 'Academic Cases', value: academic.length, cls: 'purple' },
            ...
            { label: 'Pending Approval', value: pendingApprovals.length, cls: 'orange' },
          ].map(({ label, value, cls }) => (
          ...
          <div className="card-header">
            <div>
              ...
              <span style={{ marginLeft: 10, fontSize: 13, color: 'var(--gray-400)' }}>
                Academic grievances resolved by Dean/Committee — awaiting your approval
              </span>
            </div>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#7c3aed', background: '#ede9fe', padding: '4px 12px', borderRadius: 20 }}>
              {pendingApprovals.length} pending
            </span>
          </div>
          {pendingApprovals.length === 0 ? (
            <p style={{ color: 'var(--gray-400)', fontSize: 15 }}>No academic grievances awaiting approval.</p>
          ) : (
            <div className="table-container">
              <table className="table">
                <thead><tr>
                  <th>Ticket</th><th>Title</th><th>Campus</th><th>School</th><th>Resolution by Dean</th><th>Action</th>
                </tr></thead>
                <tbody>
                  {pendingApprovals.map(g => (
                    <tr key={g.id}>
                      ...
                      <td style={{ fontSize: 13 }}>{g.school || '—'}</td>
                ...
                <tbody>
                  {allGrievances.slice(0, 12).map(g => (
                    <tr key={g.id} style={{ background: g.sla_breached ? 'var(--danger-lt)' : undefined }}>
                      ...
                      <td>
                        <button className="btn btn-sm btn-primary" onClick={() => navigate(`/grievances/${g.id}`)} style={{ fontSize: 13 }}>View</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="card">
          <div className="card-header">
            ...
            <span style={{ fontSize: 13, color: 'var(--gray-400)' }}>{pending.length} awaiting assignment</span>
          </div>
          {pending.length === 0 ? (
            <p style={{ color: 'var(--gray-400)', fontSize: 15 }}>No pending grievances.</p>
          ) : (
            <div className="table-container">
              <table className="table">
                <thead><tr>
                  <th>Ticket</th><th>Title</th><th>Category</th><th>Campus</th><th>Submitted</th><th>Action</th>
                </tr></thead>
                <tbody>
